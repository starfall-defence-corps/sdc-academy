...
| 1.6 | Inventory from Nothing | Host discovery, service fingerprinting, dynamic inventory | [mission-1-6-inventory-from-nothing](https://github.com/starfall-defence-corps/mission-1-6-inventory-from-nothing) |
...
**Zero-setup option**: every mission repo except `mission-3-5-battle-rattle` ships a devcontainer — after creating your copy, open it with **Code → Codespaces → Create codespace** and skip all the installs below. First boot takes ~5 minutes (one-time); the free tier covers ~30 hours/month on the lab machine.
